...
constexpr uint32_t PERIOD_VALUE = (uint32_t)(4000 - 1); /* Period Value  */
...
void pwmDriver::set_light_brightness(float brightness_percentage) {
    ...
    if (brightness_percentage < 0.01) {
        if (pin == GPIO_PIN_1 && port == GPIOA) {
            __HAL_TIM_SET_COMPARE(&TimHandle, TIM_CHANNEL_2, 1);
        } else if (pin == GPIO_PIN_0 && port == GPIOA) {
            __HAL_TIM_SET_COMPARE(&TimHandle, TIM_CHANNEL_1, 1);
        } else if (pin == GPIO_PIN_3 && port == GPIOC) {
        } else if (pin == GPIO_PIN_2 && port == GPIOC) {
        } else if (pin == GPIO_PIN_1 && port == GPIOC) {
        } else if (pin == GPIO_PIN_0 && port == GPIOC) {
        }
